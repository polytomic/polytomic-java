...
public class AsyncRawSchemasClient {
     ...
     */
    public CompletableFuture<PolytomicHttpResponse<SchemaFieldResponseEnvelope>> patchField(
            String connectionId, String schemaId, String fieldId) {
        return patchField(
                connectionId,
                schemaId,
    ...
    public CompletableFuture<PolytomicHttpResponse<SchemaFieldResponseEnvelope>> patchField(
        ...
        CompletableFuture<PolytomicHttpResponse<SchemaFieldResponseEnvelope>> future = new CompletableFuture<>();
        client.newCall(okhttpRequest).enqueue(new Callback() {
            @Override
            public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
                try (ResponseBody responseBody = response.body()) {
                    ...
                    if (response.isSuccessful()) {
                        future.complete(new PolytomicHttpResponse<>(
                                ObjectMappers.JSON_MAPPER.readValue(
                                        responseBodyString, SchemaFieldResponseEnvelope.class),
                                response));
                        return;
                    }
                    try {
     ...
     * </blockquote>
     */
    public CompletableFuture<PolytomicHttpResponse<Void>> refresh(String id, IdempotentRequestOptions requestOptions) {
        HttpUrl.Builder httpUrl = HttpUrl.parse(this.clientOptions.environment().getUrl())
                .newBuilder()
                .addPathSegments("api/connections")
                .addPathSegment(id)
