/**
 * This file was auto-generated by Fern from our API Definition.
 */
package com.polytomic.api.errors;

import com.polytomic.api.core.PolytomicApiException;
import com.polytomic.api.types.ApiError;
import okhttp3.Response;

public final class ConflictError extends PolytomicApiException {
    /**
     * The body of the response that triggered the exception.
     */
    private final ApiError body;

    public ConflictError(ApiError body) {
        super("ConflictError", 409, body);
        this.body = body;
    }

    public ConflictError(ApiError body, Response rawResponse) {
        super("ConflictError", 409, body, rawResponse);
        this.body = body;
    }

    /**
     * @return the body
     */
    @java.lang.Override
    public ApiError body() {
        return this.body;
    }
}
