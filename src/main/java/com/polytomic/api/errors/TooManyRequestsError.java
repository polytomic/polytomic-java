/**
 * This file was auto-generated by Fern from our API Definition.
 */
package com.polytomic.api.errors;

import com.polytomic.api.core.PolytomicApiException;
import com.polytomic.api.types.ApiError;
import okhttp3.Response;

public final class TooManyRequestsError extends PolytomicApiException {
    /**
     * The body of the response that triggered the exception.
     */
    private final ApiError body;

    public TooManyRequestsError(ApiError body) {
        super("TooManyRequestsError", 429, body);
        this.body = body;
    }

    public TooManyRequestsError(ApiError body, Response rawResponse) {
        super("TooManyRequestsError", 429, body, rawResponse);
        this.body = body;
    }

    /**
     * @return the body
     */
    @java.lang.Override
    public ApiError body() {
        return this.body;
    }
}
